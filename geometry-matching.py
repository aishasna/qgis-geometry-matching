...
class GeometryMatching(QgsProcessingAlgorithm):
    ...
    LIKELY_A_ID = ["ID", "id", "OBJECTID", "PARCEL_ID", "PARCELID", "FID", "GUID","KEY", "UID"]
    LIKELY_B_ID = ["ID", "id", "OBJECTID", "PARCEL_ID", "PARCELID", "FID", "GUID","KEY", "UID"]

    def initAlgorithm(self, config=None):
        self.addParameter(
            QgsProcessingParameterFeatureSource(self.INPUT_DATASET_A, 'Layer Dataset A', [QgsProcessing.TypeVectorPolygon])
        )
        self.addParameter(
            QgsProcessingParameterFeatureSource(self.INPUT_DATASET_B, 'Layer Dataset B', [QgsProcessing.TypeVectorPolygon])
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        thr_all = self.parameterAsDouble(parameters, self.THR_OVERLAP, context) 
        
        if DatasetA_layer is None or DatasetB_layer is None:
            raise QgsProcessingException('Input layer invalid')

        # 1. Parameter Initialization
        thr_strong = thr_all 
        thr_medium = thr_all 
        thr_percent = int(thr_all * 100)
        ...
        for i, (dataset_b_fid, (gs, pid, a_s)) in enumerate(B_feats.items()):
            if feedback.isCanceled(): return {}
            if a_s <= 0: continue
            
            bbox_ids = A_index.intersects(gs.boundingBox())
            
            for dataset_a_fid in bbox_ids:
                gb, bid, a_b = A_feats[dataset_a_fid] 
                if a_b <= 0: continue
                ...
                if a_int > 0: 
                    ...
                    edges.append({
                        "Dataset_B_ID": pid, "Dataset_A_ID": bid,
                        "R_Dataset_B": r_b, "R_Dataset_A": r_a
                    })
        ...
        for e in edges:
            is_relevant = (e["R_Dataset_B"] >= thr_medium) or (e["R_Dataset_A"] >= thr_medium)
            
            if is_relevant:
                partners_a_by_b[e["Dataset_B_ID"]].add(e["Dataset_A_ID"])
                partners_b_by_a[e["Dataset_A_ID"]].add(e["Dataset_B_ID"])

        for e in edges:
            pid, bid = e["Dataset_B_ID"], e["Dataset_A_ID"]
            e["deg_dataset_b"] = len(partners_a_by_b[pid])
            e["deg_dataset_a"] = len(partners_b_by_a[bid])
        ...
        for e in edges:
            ...
            is_strong = (e["R_Dataset_B"] >= thr_strong) and (e["R_Dataset_A"] >= thr_strong)
            is_1_1_deg = (e["deg_dataset_b"] == 1) and (e["deg_dataset_a"] == 1) 
            if is_strong and is_1_1_deg:
                status_parcel_a[bid] = "1:1"
                status_parcel_b[pid] = "1:1"
        # PASS 2: 1:M (Split Parcel B) dan M:1 (Merge Parcel A)
        for e in edges:
            pid, bid = e["Dataset_B_ID"], e["Dataset_A_ID"]
            if status_parcel_a.get(bid) == "1:1" or status_parcel_b.get(pid) == "1:1": continue
            is_split_parcel_b_deg = (e["deg_dataset_a"] == 1) and (e["deg_dataset_b"] > 1)
            is_split_parcel_b_thr = (e["R_Dataset_A"] >= thr_medium)
            if is_split_parcel_b_deg and is_split_parcel_b_thr:
        ...
        for e in edges:
            ...
            is_merge_parcel_a_deg = (e["deg_dataset_a"] > 1) and (e["deg_dataset_b"] == 1)
            is_merge_parcel_a_thr = (e["R_Dataset_B"] >= thr_medium)
            if is_merge_parcel_a_deg and is_merge_parcel_a_thr:
        ...
        count_a = Counter(status_parcel_a.values())
        count_b = Counter(status_parcel_b.values())
        count_a["No Match"] = total_A_features - len(status_parcel_a)
        count_b["No Match"] = total_B_features - len(status_parcel_b)
        
        # 7. Writing Outputs (3 Output Sinks)
        
        # --- A. Sink Dataset A (Geometri) ---
        A_fields = DatasetA_layer.fields()
        A_fields.append(QgsField("Classification", QVariant.String, "string", 50))
        ...
        B_fields.append(QgsField("Classification", QVariant.String, "string", 50))
        ...
        write_stats("Dataset A", total_A_features, count_a)
        write_stats("Dataset B", total_B_features, count_b)

        # 8. Final Return
        feedback.pushInfo(f"Analysis Complete. Statistical results have been saved to the Comparative Statistics Table layer (THR={thr_percent}%)")
